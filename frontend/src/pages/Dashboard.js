...
import { FaCheckCircle, FaEdit, FaArchive, FaRocket } from "react-icons/fa";
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchEventDetails = async () => {
      ...
      const response = await fetch(
        `${process.env.REACT_APP_API_URL}/api/mainroutes/admin`,
        {
          headers: {
  ...
  const upcomingEvents =
    eventdetails?.filter(
      (event) =>
        new Date(event.event_date) >= new Date() && event.status !== "archived",
    ).length || 0;
  ...
  return (
    <>
      <button className="hamburger1" onClick={() => setMenuOpen(true)}>
        <FaBars />
      </button>
      ...
      <div className={`sidebar1 ${menuOpen ? "active" : ""}`}>
        ...
        <div className="sidebar1-links">
          ...
          <Link onClick={() => setMenuOpen(false)} to="/formOne">
            Create Event
          </Link>
        </div>
      </div>
      ...
      {menuOpen && (
        <div className="overlay1" onClick={() => setMenuOpen(false)} />
      )}
      ...
      <div className="dash-container">
        {activeEvents.length === 0 ? (
          <div className="empty-state">
            ...
            <button className="empty-btn" onClick={() => navigate("/formOne")}>
              + Create First Event
            </button>
          </div>
        ) : (
              ...
              return (
                ...
                >
                  ...
                  <span className={`status-badge ${eventdetail.status}`}>
                    {eventdetail.status.charAt(0).toUpperCase() +
                      eventdetail.status.slice(1)}
                  </span>
                  ...
                  <p>
                    <FaClock />{" "}
                    {new Date(
                      `1970-01-01T${eventdetail.start_time}`,
                  ...
                  <button
                    onClick={(event) => handleArchive(event, eventdetail._id)}
                  >
                    Archive
                  </button>
                  ...
                  {eventdetail.status === "draft" && (
                    <button
                      onClick={(e) =>
                        handleRestore(e, eventdetail._id, "published")
                      }
                    >
                      Publish
                    </button>
